...
uint256 constant PORTAL_GENERATXORS_NUM = 10;


struct Generatxor {
    uint16[EQUIPPED_WEARABLE_SLOTS] equippedWearables; //The currently equipped wearables of the Generatxor
    ...
    uint256 usedSkillPoints; //The number of skill points this NFT has already used
    uint256 interactionCount; //How many times the owner of this NFT has interacted with it.
    address collateralType;
    uint40 claimTime; //The block timestamp when this NFT was claimed
    uint40 lastTemporaryBoost;
    uint16 hauntId;
    uint16 tbdId;
    address owner;
    uint8 status; // 0 == portal, 1 == VRF_PENDING, 2 == open portal, 3 == Generatxor
    uint40 lastInteracted; //The last time this Generatxor was interacted with
    bool locked;
    address escrow; //The escrow address this Generatxor manages.
}
...
struct ItemType {
    ...
    uint16 minLevel; //The minimum Generatxorlevel required to use this item. Default is 1.
...
struct GeneratxorCollateralTypeInfo {
...
struct ERC721Listing {
    ...
    uint256 category; // 0 is closed portal, 1 is vrf pending, 2 is open portal, 3 is Generatxor
    uint256 priceInWei;
    uint256 timeCreated;
...
struct AppStorage {
    mapping(address => GeneratxorCollateralTypeInfo) collateralTypeInfo;
    ...
    mapping(uint256 => Generatxor) generatxors;
    ...
    mapping(string => bool) generatxorNamesUsed;
...
contract Modifiers {
    ...
    modifier onlyGeneratxorOwner(uint256 _tokenId) {
        require(LibMeta.msgSender() == s.generatxors[_tokenId].owner, "LibAppStorage: Only generatxor owner can call this function");
        _;
    }
    modifier onlyUnlocked(uint256 _tokenId) {
        require(s.generatxors[_tokenId].locked == false, "LibAppStorage: Only callable on unlocked generatxors");
        _;
    }
