...
import {LibAppStorage, AppStorage, ItemType, Generatxor, EQUIPPED_WEARABLE_SLOTS} from './LibAppStorage.sol';
...
library LibItems {
    ...
    function removeFromParent(
        ...
        if (_fromContract == address(this) && bal == 1) {
            Generatxor storage generatxor = s.generatxors[_fromTokenId];
            if (
                generatxor.equippedWearables[LibItems.WEARABLE_SLOT_HAND_LEFT] == _id &&
                generatxor.equippedWearables[LibItems.WEARABLE_SLOT_HAND_RIGHT] == _id
            ) {
    ...
    function checkWearableIsEquipped(uint256 _fromTokenId, uint256 _id) internal view {
        ...
        for (uint256 i; i < EQUIPPED_WEARABLE_SLOTS; i++) {
            require(s.generatxors[_fromTokenId].equippedWearables[i] != _id, "Items: Cannot transfer wearable that is equipped");
        }
    }
}